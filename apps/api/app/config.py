...
def read_float(
    ...
    try:
        ...
        raise RuntimeError(
            f"{variable_name} doit contenir un nombre."
        ) from error
...
class Config:
    ...
    PROJECT_ARCHIVE_ROOT = os.getenv(
        ...
        str(DEFAULT_PROJECT_ARCHIVE_ROOT),
    )

    PROJECT_ARCHIVE_MAX_BYTES = read_integer(
        "PROJECT_ARCHIVE_MAX_BYTES",
        100 * 1024 * 1024,
    )

    PROJECT_ARCHIVE_MAX_ENTRIES = read_integer(
        "PROJECT_ARCHIVE_MAX_ENTRIES",
        20_000,
    )

    PROJECT_ARCHIVE_MAX_UNCOMPRESSED_BYTES = read_integer(
        "PROJECT_ARCHIVE_MAX_UNCOMPRESSED_BYTES",
        500 * 1024 * 1024,
    )

    PROJECT_ARCHIVE_MAX_COMPRESSION_RATIO = read_float(
        "PROJECT_ARCHIVE_MAX_COMPRESSION_RATIO",
        100.0,
    )

    # Marge supplémentaire pour les métadonnées multipart.
    MAX_CONTENT_LENGTH = (
        PROJECT_ARCHIVE_MAX_BYTES
        + 2 * 1024 * 1024
    )